...
### This programs reports the mutation rates, coverage and number of mutations at each positions in a provided bedfile
...
my $USAGE = "$0 --bed clusters.bed --EditedMatrix DART.matrix.gz --outfile output_file.out\n";

###set option of program using Getopt::long
my ($bedFile, $tablename, $OUTFILE, $verbose, $help,$extUTR)='';
my $scratch = "./SCRATCH";
...
if ( $help ) { error_out(); } ### help incorporate in next block

# if arguments are missing, print list of arguments and exit ### does not check for tp since they may be 0 wich would crash the program

unless($bedFile && $tablename)
{
	say STDERR "Error. Please provide a Bed file and an coverage matrix";
	error_out();
}

### check for all files before proceeding ###
if($bedFile and ! -f $bedFile){say "Error. Bed file $bedFile could not be found";exit();}
unless(-f $tablename){say "Error, could not find edited matrix $tablename";exit();}
###		

### check which type of chr in annotation file
my $cluster_stem ="";
if ($bedFile){$cluster_stem = check_chr($bedFile, 0);}
...
if ($verbose)
{
	say STDERR "Processing files $bedFile and $tablename";
...
my $hr_genes = {};
open(my $annotation_handle,"sort -k1,1 -k2,2n $bedFile |") or die "Can't open $bedFile: $!\n";  
...
quantify site reports the mutation rates, coverage and number of mutations at each positions in a matrix file given the provided list of edit sites.

Options: 

	--bed	: Cluster bed file containing cluster region to analyze (s)
	--EditedMatrix		: DART RNA count matrix (s)
	--editType		: Conversion to quantify in the following format : C2U (default) or A2I (s)
	--minEdit		: minimal editing percentage for sites to be considered default to 2.5%. please write as a number without % symbol (f)
	--maxEdit		: maximum editing percentage for sites to be considered default to 95%. please write as a number without % symbol (f)
